...
router.get('/', async (req, res) => {
  try {
    const { rows } = await query(
      `SELECT id, to_char(sana, 'YYYY-MM-DD') AS sana, gaz, svet, obed, ishchi, boshqa, izoh
...
// Kirish: { sana: 'YYYY-MM-DD', gaz, svet, obed, ishchi, boshqa, izoh }
router.put('/', requireRole('Owner'), async (req, res) => {
  try {
    const { sana, gaz = 0, svet = 0, obed = 0, ishchi = 0, boshqa = 0, izoh = '' } = req.body || {};
    if (!sana || !/^\d{4}-\d{2}-\d{2}$/.test(String(sana))) {
      return res.status(400).json({ error: 'sana YYYY-MM-DD formatida bo\'lishi kerak' });
    }
    const num = (v) => Math.max(0, Math.round(Number(v) || 0));
    const { rows } = await query(
      `INSERT INTO xarajatlar (tenant_id, sana, gaz, svet, obed, ishchi, boshqa, izoh)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       ON CONFLICT (tenant_id, sana) DO UPDATE
         SET gaz = EXCLUDED.gaz, svet = EXCLUDED.svet, obed = EXCLUDED.obed,
             ishchi = EXCLUDED.ishchi, boshqa = EXCLUDED.boshqa, izoh = EXCLUDED.izoh
       RETURNING id, to_char(sana, 'YYYY-MM-DD') AS sana, gaz, svet, obed, ishchi, boshqa, izoh`,
      [req.user.tenant_id, sana, num(gaz), num(svet), num(obed), num(ishchi), num(boshqa), String(izoh || '')]
    );
    res.json(rows[0]);
  } catch (err) {
